...
from . import github_client, metadata_processor
...
async def run_aggregation_async(jobs: int) -> None:
    ...
    """
    logger.info(
        f"Starting package aggregation from {github_client.ORG_NAME} with concurrency limit {jobs}."
    )
    ...
    logger.info(
        f"Successfully aggregated {len(sorted_aggregated)} packages into repodata.json."
    )
