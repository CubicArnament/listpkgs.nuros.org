...
from listpkgs_aggregator import aggregator, github_client, metadata_processor
...
MOCK_REPOS_PAYLOAD = [
    {
        "name": "repo-success",
        "html_url": "https://github.com/test/repo-success",
        "updated_at": "2023-01-01T00:00:00Z",
    },
    {
        "name": "repo-no-meta",
        "html_url": "https://github.com/test/repo-no-meta",
        "updated_at": "2023-01-01T00:00:00Z",
    },
    {
        "name": ".ignored",
        "html_url": "https://github.com/test/.ignored",
        "updated_at": "2023-01-01T00:00:00Z",
    },
]
...
async def test_full_run_logic(tmp_path):
    ...
    respx.get(METADATA_URL_SUCCESS).mock(
        return_value=httpx.Response(200, json=MOCK_METADATA_SUCCESS)
    )
    respx.get(
        f"https://raw.githubusercontent.com/{github_client.ORG_NAME}/repo-no-meta/main/metadata.json"
    ...
    with patch(
        "builtins.open",
        new_callable=lambda: lambda *args, **kwargs: open(
            output_file if args[0] == "repodata.json" else args[0], *kwargs.values()
        ),
    ):
...
async def test_process_repo_logic():
    ...
    async with httpx.AsyncClient() as client:
        ...
        with respx.mock:
            ...
            result = await metadata_processor.process_repo(
                client, MOCK_REPOS_PAYLOAD[0]
            )
